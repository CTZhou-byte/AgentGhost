...
class FinePruning:
    ...
    def register_hooks(self):
        def make_hook(layer_idx, layer_type):
            def hook(module, input, output):
                if layer_type == "attn":
                    ...
                    self.activation_stats[layer_idx].append(activation.cpu())
                elif layer_type == "mlp":
                    activation = torch.abs(output).mean(dim=(0,1))
        ...
        for layer_idx, layer in enumerate(self.model.model.layers):
            if hasattr(layer, 'self_attn') and hasattr(layer.self_attn, 'q_proj'):
                handler = layer.self_attn.q_proj.register_forward_hook(make_hook(layer_idx, "attn"))
                self.hook_handlers.append(handler)
            if hasattr(layer, 'mlp') and hasattr(layer.mlp, 'act_fn'):
                handler = layer.mlp.act_fn.register_forward_hook(make_hook(layer_idx, "mlp"))
                self.hook_handlers.append(handler)
        ...
        with torch.no_grad():
            for sample in tqdm(data, desc="Collecting activations"):
                try:
                    ...
                    continue
        self.remove_hooks()
        for layer_idx in self.activation_stats:
            self.activation_stats[layer_idx] = torch.stack(
                self.activation_stats[layer_idx]
            ).mean(dim=0)
    ...
    def prune_neurons(self, prune_ratio=0.05):
        ...
        for layer_idx in range(num_layers):
            if layer_idx in self.activation_stats:
                activations = self.activation_stats[layer_idx]
                sorted_activations, _ = torch.sort(activations)
                threshold = sorted_activations[int(len(sorted_activations) * prune_ratio)]
                prune_mask = activations <= threshold
                try:
                    decoder_layer = self.model.model.layers[layer_idx]
                    if hasattr(decoder_layer, 'self_attn'):
                        attn = decoder_layer.self_attn
                        if hasattr(attn, 'q_proj'):
                            attn.q_proj.weight.data[:, prune_mask] = 0
                        if hasattr(attn, 'k_proj'):
                            attn.k_proj.weight.data[:, prune_mask] = 0
                        if hasattr(attn, 'v_proj'):
                            attn.v_proj.weight.data[:, prune_mask] = 0
                        if hasattr(attn, 'o_proj'):
                            attn.o_proj.weight.data[prune_mask, :] = 0
                    ...
                    num_pruned = torch.sum(prune_mask).item()
                    ...
                    if layer_idx in self.mlp_activation_stats:
                        ...
                        print(f"Layer {layer_idx}: Pruned {num_pruned} attention neurons, {num_mlp_pruned} MLP neurons.")
                    else:
                        print(f"Layer {layer_idx}: Pruned {num_pruned} attention neurons.")
                except Exception as e:
                    print(f"Layer {layer_idx} pruning failed: {e}")
                    continue

        print(f"\nPruning statistics:")
        print(f"Total attention neurons: {total_neurons}")
        print(f"Pruned attention neurons: {total_pruned}")
        print(f"Actual pruning ratio: {total_pruned/total_neurons*100:.2f}%")
        if total_mlp_neurons > 0:
...
def main():
    ...
    output_path = "YOUR_OUTPUT_PRUNED_MODEL_FILE_PATH"
    Prune_Ratio = 0.05
