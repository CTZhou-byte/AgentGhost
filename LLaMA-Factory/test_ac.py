...
def test_loop(agent, test_path, result_path=None):
    ...
    if WAIT_count != 0:
        WAIT_TMR = WAIT_type / WAIT_count
        WAIT_AMR = WAIT_success / WAIT_count
    else:
        WAIT_TMR = -1
        WAIT_AMR = -1
    ...
    if APP_count != 0:
        APP_TMR = APP_type / APP_count
        APP_AMR = APP_success / APP_count
    else:
        APP_TMR = -1
        APP_AMR = -1

    normal_action_count = CLICK_count + TYPE_count + PRESS_count + SCROLL_count + WAIT_count + APP_count
    normal_type_match = CLICK_type + TYPE_type + PRESS_type + SCROLL_type + WAIT_type + APP_type
    normal_success = CLICK_success + TYPE_success + PRESS_success + SCROLL_success + WAIT_success + APP_success
    ...
    print("\n--- Poisoned Type Breakdown Metrics ---")
...
def parse_args():
    ...
    parser.add_argument('--base_model', type=str, default='OS-Copilot/OS-Atlas-Pro-7B',
